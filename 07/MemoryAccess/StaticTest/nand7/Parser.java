...
public class Parser {
	...
	private static final int STATIC_SEGMENT_FIRST_REG = 16;
		...
		public void handleOperation(Command command) {
			...
			if (command.isPush()) {
				switch (command.getMemorySegmentType()) {
					...
					case STATIC:
					case TEMP:
						int baseRegister = command.getMemorySegmentType().equals(MemorySegmentType.TEMP) ? TEMP_SEGMENT_FIRST_REG : STATIC_SEGMENT_FIRST_REG;
						int registerNum = baseRegister + command.getValue();
						builder.append("// Performing temp/static push").append(NEW_LINE_CHAR);
